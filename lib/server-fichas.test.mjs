...
test("duplicate protection creates a new ficha while updating client data on related fichas", async () => {
  ...
  const createService = await readFile(new URL("./fichaCreateService.ts", import.meta.url), "utf8")
  ...
  assert.match(collectionRoute, /resolution\?\.action === "overwrite_client"/)
  assert.match(collectionRoute, /mergeFichaClients\(ficha\.id, \[ficha\.id, \.\.\.relatedFichaIds\], consultor\)/)
  assert.match(collectionRoute, /ficha = await getFichaById\(ficha\.id\)/)
  assert.doesNotMatch(createService, /updateFicha\(resolution\.matchedFichaId/)
...
test("creates ficha numbers from the highest existing client number instead of row count", async () => {
  ...
  assert.match(server, /getFichaSequenceByCpf\(data\.cpfCnpj\)/)
  assert.match(server, /if \(!cpfNormalizado\) return 1/)
...
test("merges selected client records through one atomic database operation", async () => {
  ...
  assert.match(migration, /client_group_id uuid/i)
  assert.match(migration, /group_id_value := coalesce\(principal\.client_group_id, gen_random_uuid\(\)\)/i)
  assert.match(migration, /update public\.fichas_venda set numero_ficha = null[\s\S]*client_group_id = group_id_value/i)
  assert.match(migration, /row_number\(\) over \(order by data_contrato nulls last, created_at nulls last, id\)/i)
  assert.match(migration, /set numero_ficha = pending\.position/i)
})

test("persists and reads third-party contact fields", async () => {
  const server = await readFile(new URL("./server-fichas.ts", import.meta.url), "utf8")

  assert.match(server, /telefone_terceiros: normalizedData\.telefoneTerceiros/)
  assert.match(server, /email_terceiros: normalizedData\.emailTerceiros/)
  assert.match(server, /telefoneTerceiros: String\(row\.telefone_terceiros/)
  assert.match(server, /emailTerceiros: String\(row\.email_terceiros/)
})

test("round-trips the third-party ownership flag and CPF", async () => {
  const server = await readFile(new URL("./server-fichas.ts", import.meta.url), "utf8")

  assert.match(server, /placa_proprietario: normalizedData\.placaProprietario \|\| null/)
  assert.match(server, /cpf_proprietario: normalizedData\.cpfProprietario \|\| null/)
  assert.match(server, /placaProprietario: normalizeOwnerFlags\(/)
  assert.match(server, /cpfProprietario: String\(row\.cpf_proprietario \?\? ""\)/)
})
