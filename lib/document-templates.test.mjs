...
test("document summaries omit deadlines and expose the requested process and fine fields", () => {
  ...
  assert.equal((documentTemplatesSource.match(/\.join\("<br>"\)/g) || []).length, 2)
})
...
test("uses the contract date for Data Ficha", () => {
  assert.match(documentTemplatesSource, /dataFicha: formatFichaCreatedDate\(values\.dataContrato \|\| ""\)/)
})

test("renders civil status with only its first letter uppercase", () => {
  assert.match(documentTemplatesSource, /estadoCivil: formatSentenceCase\(values\.estadoCivil\)/)
})

test("renders each process and fine summary on its own HTML line", () => {
  assert.equal((documentTemplatesSource.match(/\.join\("<br>"\)/g) ?? []).length >= 2, true)
})

test("exposes variables for third-party contact details", () => {
  assert.match(documentTemplatesSource, /nomeTerceiros: values\.terceiros/)
  assert.match(documentTemplatesSource, /telefoneTerceiros: values\.telefoneTerceiros/)
  assert.match(documentTemplatesSource, /emailTerceiros: values\.emailTerceiros/)
})
