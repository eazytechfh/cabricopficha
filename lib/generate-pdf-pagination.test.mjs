...
test("nao quebra linha nos campos Instancia, Tipo e Prazo das multas e processos no PDF", async () => {
  const source = await readFile(new URL("../components/FichaPdf.tsx", import.meta.url), "utf8")

  assert.match(source, /gridRow\("1\.2fr 1\.2fr 0\.9fr 1fr 1\.1fr 0\.65fr"/)
  assert.match(source, /field\("Tipo", line\.tipoProcesso\)/)
  assert.match(source, /field\("Multas", normalizeMultasProcessoLabels\(line\.multasProcesso, true\)\)/)
  assert.doesNotMatch(source, /field\("Tipo do Processo", line\.tipoProcesso\)/)
  assert.doesNotMatch(source, /field\("Multas do Processo", normalizeMultasProcessoLabels/)
  assert.match(source, /nowrapField\("Instância", formatInstanciaLabel\(line\.instanciaMulta\)\)/)
  assert.match(source, /nowrapField\("Tipo", line\.tipoMulta\)/)
  assert.match(source, /nowrapField\("Prazo", formatDate\(line\.prazoMulta\)\)/)
  assert.match(source, /nowrapField\("Instância", formatInstanciaLabel\(line\.instanciaProcesso\)\)/)
