...
test("admins can select client groups and merge them into a chosen primary record", async () => {
  ...
  assert.match(source, /primaryMergeFicha[\s\S]*Data de criação[\s\S]*formatDisplayDate\(primaryMergeFicha\.dataContrato/)
  assert.match(source, /Consultor[\s\S]*primaryMergeFicha\.nomeConsultor/)
