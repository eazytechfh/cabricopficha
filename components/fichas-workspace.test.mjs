...
test("admins can select client groups and merge them into a chosen primary record", async () => {
  ...
  assert.match(source, /primaryMergeGroup[\s\S]*Data de criação[\s\S]*formatDisplayDate\(primaryMergeGroup\.contratos\[0\]\?\.dataContrato/)
  assert.match(source, /Consultor[\s\S]*primaryMergeGroup\.contratos\[0\]\?\.nomeConsultor/)
