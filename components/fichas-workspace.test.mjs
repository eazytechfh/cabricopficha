...
test("consulted ficha shows the contract date as the ficha date", async () => {
  ...
  assert.match(source, /orderedNumbers\.every\(\(number, index\) => number === index \+ 1\)/)
  assert.match(source, /selectedFichaNumbers\.get\(contrato\.id\)/)
  assert.match(source, /dataContrato \|\| "9999-12-31"/)
...
test("admins can select client groups and merge them into a chosen primary record", async () => {
  ...
  assert.match(source, /Todas as fichas, contratos, pagamentos e históricos abaixo serão preservados/)
  assert.match(source, /mergeAllContratos = useMemo/)
  assert.match(source, /Resumo da junção/)
  assert.match(source, /isPrimary = ficha\.id === mergePrimaryFichaId/)
  assert.match(source, /Será mesclada/)
